 ...
 */
#define centerErrorTolerance 18  // 대략이정도 count => 0.15*signal period/sampling period
#define errorTolerance       40 //% 대략이정도 duty difference/3
...
void loop(){
  ...
  if(record_state == 1){
    if((((int)(count_period - count_temp+centerErrorTolerance))>count_temp) 
            & ((count_period - count_temp)<(int)(centerErrorTolerance+count_temp))){end_of_data =1;}
    else {(int)((0.01*errorTolerance*(count_period - count_temp)))>count_temp?
